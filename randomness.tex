...
Our randomness expansion protocol, Protocol~1, is described in Figure~\ref{fig:protocol}. The protocol is parametrized by a choice of security parameter $\lambda$. All other parameters are assumed to be specified as a function of $\lambda$: the number of rounds $N$, the error tolerance parameter $\gamma \geq 0$, and the testing parameter $q\in (0,1]$. In addition, the protocol depends on a TCF family (see Definition~\ref{def:trapdoorclawfree}) that is known to both the verifier and the prover.
...
Depending on the round type, the verifier chooses a challenge $C\in\{0,1\}$ that she sends to the prover. In the case of a test round the challenge is chosen uniformly at random; in the case of a generation round the challenge is always $C=1$. In case $C=0$ the prover is asked to return a pair $(m,d)\in \{0,1\}\times\{0,1\}^w$. The answer is called valid if $m=d\cdot(\inj(x_0)\oplus \inj(x_1))$ and $d\in\hat{C}_{k}$\tnote{check set notation}, where $x_0$ and $x_1$ are such that $f_{k,b}(x_b)=y$ for $b\in \{0,1\}$. Note that the verifier is able to compute $x_0$ and $x_1$ from $y$ using the trapdoor $t_k$. In case $C=1$, the prover should return  a pair $(b,x)\in \{0,1\}\times\mX$. The answer is called valid if $f_{k,b}(x)=y$. The set of valid answers on challenge $C=c\in\{0,1\}$ is denoted $V_{y,c}$. 

At the end of the protocol, the verifier computes the fraction of test rounds in which the prover has returned valid answers. If this fraction is smaller than $(1-\gamma)$, the verifier aborts. Otherwise, the verifier returns the concatenation of the prover's answer bits $b$ in generation rounds.
...
\begin{enumerate}
\item In case $C_i=0$ the prover returns $(m,d)\in\{0,1\}\times \{0,1\}^w$. If $d\notin C_{k,0,x_0}\cap C_{k,1,x_1}$ the verifier sets $W$ to a uniforly random bit. Otherwise, the verifier sets $W=1$ if $d\cdot (\inj(\hat{x}_0)\oplus \inj(\hat{x}_1)) = m$ and $W=0$ if not.
...
For purposes of analysis only we introduce a simplified form of Protocol~1, which is specified in Figure~\ref{fig:protocol2}. We call it the \emph{simplified protocol}, or Protocol~2. The protocol is very similar to the randomness expansion protocol described in Figure~\ref{fig:protocol}, except that the prover's answers and the verifier's checks are simplified. For the case of a challenge $C=0$, in Protocol~1 the prover returns an equation $(d,m)$. In the simplified protocol the prover returns a single bit $u\in\{0,1\}$ that indicates whether the equation is valid or not, i.e. $u=1$ if and only if $(m,d)\in V_{y,0}$. For the case of a challenge $C=1$, in Protocol~1 the prover returns a pair $(b,x)$. In the simplified protocol the prover returns a value $v\in\{0,1,2\}$ that is such that $v=b$ in case $(b,x)$ is valid, i.e. $(b,x)\in V_{y,1}$, and $v=2$ otherwise. 

Note that this behavior for the prover is not necessarily efficient. Moreover, it is easy for a prover to succeed in Protocol 2, e.g. by always returning $u=1$ (valid equation) and $v\in\{0,1\}$ (valid pre-image). Our analysis will not consider arbitrary provers in Protocol~2, but instead provers whose measurements satisfy certain incompatibility constraints that arise from the analysis of Protocol~1. For such provers, it will be impossible to succeed in the protocol without generating randomness. Further details are given in Section~\ref{sec:soundness}.


\subsection{Completeness}
\label{sec:completeness}

\tnote{Section needs to be updated with TCF definition}
We describe the intended behavior for the prover in the protocol. In each round, the ``honest'' prover performs the following actions:
\begin{enumerate}
\item Upon receiving $k$ from the verifier, the prover initializes a private register to the state 
$$\ket{\psi^{(0)}} \,=\, \sum_{s\in \mD_k\, e\in \mE_k}\sqrt{\chi(s,e)} \ket{s,e}_\reg{S}\;,$$
where $\chi$ is the distribution on $\mD_k\times\mE_k$ generated by $\SAMP(k)$.  
\item The prover computes $f_k$ in an ancilla register. This leads to the state 
$$\ket{\psi^{(1)}} \,=\, \sum_{s\in \mD_k\, e\in \mE_k} \sqrt{\chi(s,e)}  \ket{s,e}_\reg{S}\ket{f_k(s;e)}_\reg{T} \;.$$
\item The prover erases $e$ using the randomness recoverability property. He measures register $\reg{T}$ to obtain a vector $t\in \mR_k$ that he returns to the verifier. Then, the prover uses the unique identifiability property to tag the pre-images of $t$ as $(b,s_b)$, storing $b$ in a single-qubit register $\reg{Q}$. The prover's re-normalized post-measurement state is
$$\ket{\psi^{(2)}} \,=\, \frac{1}{\sqrt{2}}\big(\ket{0}_\reg{Q}\ket{s_0}_\reg{S}+\ket{1}_\reg{Q}\ket{s_1}_\reg{S}\big)\ket{t}_\reg{T}\;.$$
\begin{enumerate}
\item In case $C_i=1$, the prover measures registers $\reg{QS}$ in the computational basis. He obtains $(b,s_b)$ and returns it to the verifier. 
\item In case $C_i=0$, the prover evaluates the function $G_k$ on register $\reg{S}$, containing $s_b$, and then applies a Hadamard to all $n'+1$ qubits in $\reg{QS}$. He obtains the state 
\begin{align*}
\ket{\psi^{(3)}} &= 2^{-\frac{n'+2}{2}}  \sum_{d,b,m} (-1)^{d\cdot G_k(s_b)\oplus mb} \ket{m}_\reg{Q}\ket{d}_\reg{S}\\
&= (-1)^{G_k(s_0)} 2^{-\frac{n'}{2}}  \sum_{d} \ket{d\cdot (G_k(s_0)\oplus G_k(s_1))}_\reg{Q}\ket{d}_\reg{S}\;.
\end{align*}
The prover measures registers $\reg{QS}$ to obtain $(m,d)$ that it sends back to the verifier. 
\end{enumerate}
\end{enumerate}


\begin{lemma}
For any integer $n$ and $k\leftarrow\GEN(1^n)$, the strategy for the honest prover (on input $k$) in one round of the protocol can be implemented in time polynomial in $n$ and is accepted with probability at least $1-2^{-n'}$.  
\end{lemma}

\begin{proof}
Both efficiency and correctness of the prover follow from the definition of a TCF (Definition~\ref{def:trapdoorclawfree}). The prover fails only if he obtains the outcome $d=0$, which happens with probability $2^{-n'}$ in any given round. 
\end{proof}
...
We model an arbitrary prover in the randomness expansion protocol as a \emph{device} that implements the actions of the prover: the device first returns an $y\in\mY$, then, depending on the challenge $C\in\{0,1\}$, it either returns an equation $(m,d)$ (case $C=0$), or a candidate pre-image $(b,x)$ (case $C=1$). For simplicity we assume that the device makes the same set of measurements in each round of the protocol. This is without loss of generality, as we do allow the state of the device to change from one round to the next, and in particular it may play the role of a quantum memory used as a control register by the measurements. 

In Section~\ref{sec:devices} we introduce our notation for modeling devices. In Section~\ref{sec:binary} we consider a simplified model of device, that is appropriate for modeling a prover in the simplified protocol, Protocol~2. Finally in Section~\ref{sec:sim-dev} we give a reduction showing how to construct a specific simplified device from any general device, such that the simplified device generates similar transcripts in Protocol~2 as the original device does in Protocol~1. 
...
\begin{enumerate}
\item A (not necessarily normalized) positive semidefinite $\phi \in \Pos(\mH_\reg{E}\otimes \mH_\reg{Y})$. Here $\mH_\reg{E}$ is an arbitrary space private to the device, and $\mH_{\reg{Y}}$ is a space of the same dimension as the cardinality of the set $\mY$. 
...
\item For every $y\in\mY$, a projective measurement $\{M_y^{(m,d)}\}$ on $\mH_\reg{E}$, with outcomes $(m,d)\in \{0,1\}\times \{0,1\}^w$. We define 
...
Let $D = (\phi,\Pi,M)$ denote a device. When the protocol is executed, the device operates as follows. It is initialized in state $\phi$. Note that $\phi$ may represent the state of the device at an arbitrary round in the protocol, including a memory of its actions in previous rounds. When a round of the protocol is initiated, the device measures register $\reg{Y}$ in the computational basis and returns the outcome $y\in\mY$. Note that we alwyas assume that the device directly measures the register, as any pre-processing unitary can be incorporated  in the definition of the state $\phi$. When sent challenge $C=0$ (resp. $C=1$), the device measures register $\reg{E}$ using the projective measurement $\{M_y^{(m,d)}\}$ (resp. $\{\Pi_y^{(b,x)}\}$), and returns the outcome to the verifier. 
...
Using the definition of a TCF the device associated with the ``honest'' prover described in Section~\ref{sec:completeness} is efficient. 
...
\begin{enumerate}
\item $\phi\in \Pos(\mH_\reg{E})$ is a positive semidefinite operator on an arbitrary space $\mH_\reg{E}$; 
\item $\{M^0,M^1\}$ and $\{\Pi^0,\Pi^1,\Pi^2 \}$  are projective measurements on $\mH_\reg{E}$.
\end{enumerate}
 To the device we associate the post-measurement states
$$\forall u\in\{0,1\},\quad \phi_0^u\,=\, M^u \phi M^u\;,\qquad\text{and}\qquad \forall v\in\{0,1,2\},\quad\phi_1^v \,=\, \Pi^v \phi \Pi^v\;.$$
\end{definition}

A simplified device can be used in the simplified protocol in a straightforward way: upon receipt of a challenge $C=0$ (resp. $C=1$), the device performs the projective measurement $\{M^0,M^1\}$ (resp. $\{\Pi^0,\Pi^1,\Pi^2 \}$) and returns the outcome $u\in\{0,1\}$ (resp. $v\in\{0,1,2\}$) to the verifier. 
...
$$\Delta(D)\,=\,\big\|\Pi^0 M^1 \Pi^0 + \Pi^1 M^1 \Pi^1 \big\|\;.$$
\end{definition}
...
Let $C\in\{0,1\}$ be a random variable distributed as the verifier's choice of challenge in a single round of either Protocol 1 or Protocol 2. Let
\begin{align*}
\sigma(D) &= \sum_{y\in \mY}  \Pr(C=0) \proj{0}_\reg{C} \otimes\Big(  \proj{0}_\reg{O} \otimes  \sum_{(m,d) \in V_{y,0}}  M_y^{(m,d)} \phi_y M_y^{(m,d)}\\
& +   \proj{1}_\reg{O} \otimes  \sum_{(m,d) \notin V_{y,0}} 1_{d\in C_{k,0,x_0}\cap C_{k,1,x_1}}  M_y^{(m,d)} \phi_y M_y^{(m,d)} + \frac{1}{2}\Id_{\reg{O}}\otimes  \sum_{(m,d)} 1_{d\notin C_{k,0,x_0}\cap C_{k,1,x_1}}  M_y^{(m,d)} \phi_y M_y^{(m,d)} \Big)\\
...
Recall that for any $y\in\mY$, $V_{y,0}$ denotes the set of valid answers for the prover on challenge $C=0$. Let $\hat{C}_k\subseteq\{0,1\}^w$ be the set used by the verifier on challenge $C=0$. 
...
\sigma_0 &=\sum_{b\in\{0,1\}} \proj{b,x_b}_{\reg{BX}} \otimes  \sum_{(m,d) \in {V}_{y,0}} \proj{d,m}_{\reg{DM}}\otimes (\Id_\reg{Y}\otimes M_y^{(m,d)}) \rho^{(b)}_{\reg{YE}} (\Id_\reg{Y}\otimes M_y^{(m,d)})\;,\notag\\
 \sigma_1 &=\sum_{b\in\{0,1\}} \proj{b,x_b}_{\reg{BX}} \otimes  \sum_{(m,d)\notin V_{y,0}} 1_{d\in \hat{C}_{k,b,x_b} } \proj{d,m}_{\reg{DM}}\otimes (\Id_\reg{Y}\otimes M_y^{(m,d)}) \rho^{(b)}_{\reg{YE}} (\Id_\reg{Y}\otimes M_y^{(m,d)})\;.\label{eq:def-rho}
\end{align}
...
\end{equation}
 for some non-negligible function $\kappa$. For $u\in\{0,1\}$ consider the following procedure. The procedure first prepares the state $\rho$. This can be done efficiently by first preparing $\phi_{\reg{YE}}$, then measuring an $y\in \mY$, then applying the measurement $\{\Pi^{(b,x)}\}$ to $\phi_y$, and rejecting if the outcome is invalid, i.e. does not satisfy $f_{k,b}(x)=y$. 

The procedure then applies the measurement $\{M_y^{(m,d)}\}$ to $\rho$, obtaining an outcome $(d,m)$. At this point, if $d\in \hat{C}_{k,b,x_b}$ then the procedure has either prepared $\sigma_0$ or $\sigma_1$. Finally, the procedure  measures $O$ to obtain a bit $u$, and returns $(b,x_b,d,u\oplus m)$. This defines an efficient procedure. Moreover, using~\eqref{eq:bias-o} it follows  that the procedure violates the hardcore bit property (which only considers those outcome tuples from the adversary that are such that $d\in\hat{C}_{k,b,x_b}$. 
\end{proof}


\begin{proposition}\label{prop:change-d}
Let $D=(\phi,\Pi,M)$ be an efficient device. Then there is a (not necessarily efficient) elementary device $D'=(\phi,\Pi',M')$ such that $D'$ simulates $D$ up to negligible error, and $D'$ satisfies $\Delta(D') \leq \omega$, where $\omega = \frac{3}{4}$.
\end{proposition}

\begin{proof}
The proof has two steps. In the first step we argue the existence of a ``good subspace'', specified by a projection $(\Id-Q_y)$, such that the devices' measurements are strongly incompatible, when restricted to the good subspace. This is done in the following claim. 

\begin{claim}
For any $y\in \mY$, let 
$$\hat{M}_y = \sum_{(m,d):\, d\notin C_{k,0,x_0}\cap C_{k,1,x_1}} M_{y}^{(m,d)}\;,\qquad M_y = \sum_{(m,d) \in V_{y,0}} M_y^{(m,d)} + \frac{1}{2} \hat{M}_y\;,$$
and for $b \in\{0,1\}$, $\Pi_y^b = \Pi^{(b,x_b)}$.
...
Let $M=M_y$ as defined with the claim. By considering an ancilla space we may applly Naimark's theorem and consider $\{M,\Id-M\}$ %and $\{\hat{M}^{(c)},\Id-\hat{M}^{(c)}\}$, for $c\in\{0,1\}$, are 
...
\end{align}
Similarly define $\hat{\sigma}_1$ with $M$ replaced by $(\Id-M)$. Observe that $\hat{\sigma}_0 - \hat{\sigma}_1 = \sigma_0-\sigma_1$, where $\sigma_0$ and $\sigma_1$ are defined in~\eqref{eq:def-rho}.\footnote{In fact the states are a coarse-grained version of the states considered in the Lemma~\ref{lem:break}. Computational indistinguishability follows since the states considered here can be obtained efficiently from the coarse-grained states in Lemma~\ref{lem:break}.} Since by Lemma~\ref{lem:break} the latter are computationally indistinguishable, the former are as well. Using Jordan's lemma we can find a basis in which  
...
The second part of the proof of the proposition consists in defining the device $D'$. This is done as follows. The device $D'$ first measures an $y\in\mY$ exactly as $D$ would. %The device then measures $\phi_y$ using $\{Q,\Id-Q\}$, obtaining an outcome $r\in\{0,1\}$, where $r=1$ is associated with the outcome $Q$.
\begin{itemize}
\item On challenge $C=0$, the device performs the measurement $\{M_y^{(d,m)}\}$. If $d\notin C_{s,0,x_0}\cap C_{s,1,x_1}$ the device returns a random bit. Otherwise, if $(d,m)\in V_{y,0}$ it returns a $0$, and $1$ if not. %If $r=1$, the device returns a uniformly random bit. 
