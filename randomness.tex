...
\author{Zvika Brakerski\thanks{Weizmann Institute of Science, Israel. Email: \texttt{zvika.brakerski@weizmann.ac.il}.} \and Paul Christiano\thanks{OpenAI, USA. Work performed while at UC Berkeley} \and Urmila Mahadev\thanks{UC Berkeley, USA. Email: \texttt{mahadev@berkeley.edu}} \and Umesh Vazirani \and Thomas Vidick\thanks{California Institute of Technology, USA. Email: \texttt{vidick@cms.caltech.edu}}}
\date{}
\maketitle
...
\begin{abstract}
\znote{Edited. I hope it's not too pompous and that I didn't misunderstand the quantum state of the art. Original text is commented out in source file.}

The question of how a classical machine can utilize the power of a quantum worker, even in the absence of trust, is gaining much attention as experimental progress is made towards the construction of more complex quantum systems. This question has many facets, such as utilizing the quantum worker to generate certifiable randomness, device-independent quantum key distribution, or verifiable delegation of quantum computation.

The main tool for achieving these tasks as of yet has to do with the study of non-locality and Bell inequalities, which requires the classical client to access (at least) two non-communicating but entangled quantum devices. While large-scale Bell tests remain a distant experimental challenge, thus making the above setup hard to reach in an experiment, medium-scale quantum devices are being thoroughly investigated. Solutions using a single quantum device are therefore desirable.
...
protocol, the randomness distinguisher is not assumed computationally bounded, and can in principle completely break any cryptographic assumption. We provide a sketch of our security proof in Section~\ref{sec:proofsketch}. 


In terms of efficiency, 
for the specific LWE-based TCF that we construct, our protocol can use as few as $\poly\log(N)$ bits of randomness to 
generate $O(N)$ bits that are statistically within negligible distance from uniform. However, this would require assuming that the underlying LWE assumption is hard even for sub-exponential size quantum circuits (which is consistent with current knowledge). The more conservative assumption that our variant of LWE is only hard for polynomial size quantum circuits requires $O(N^{\epsilon})$ bits of randomness for generating the TCF, for any constant $\epsilon >0$. The following is an informal description; see Theorem~\ref{thm:expansion} for a more formal statement.


\begin{theorem}[Informal]
Let $\mathcal{F}$ be a TCF family and $\lambda$ a security parameter. Let $N = \Omega(\lambda^2)$. There is an $N$-round protocol for the interaction between a classical polynomial-time verifier and a quantum polynomial-time device such that the protocol can be executed using $\poly(\log(N),\lambda)$ bits of randomness, and for any efficient device and side information $\reg{R}$ correlated with the device's initial state,
$$\Hmin^{\delta'}(O|CR)_{\ol{\rho}} \geq  (\kappa-o(1)) N\;.$$
Here $\kappa$ is a positive constant, $\delta'$ is a negligible function of $\lambda$, and $\ol{\rho}$ is the final state of the classical output register $\reg{O}$, the classical register $\reg{C}$ containing the verifier's messages to the device, and the side information $\reg{R}$, restricted to transcripts that are accepted by the verifier in the protocol. \znote{I don't understand the parameters here. The number of random bits to be generated is $\poly(\lambda)$ so if we want it to be $\poly\log(N)$ then we need $\lambda = \poly\log(N)$ and not $\lambda = \sqrt{N}$.}\tnote{edited. It didn't say $\lambda = \sqrt{N}$, it said $\lambda = O(\sqrt{N}$, which is consistent, but I agree it was unclear.} 
\end{theorem}
...
We first describe the protocol in slightly more detail (see Section~\ref{sec:protocol} for a formal description). The verifier first uses $\poly(\log(N),\lambda)$ bits of randomness to 
...
The protocol then proceeds for $N$ rounds. In each round the device first outputs an image $y \in \{0,1\}^m$. After having received $y$, the verifier issues one of two 
...
uses a strong quantum-proof randomness extractor to extract $\Omega(N)$ bits of randomness from the recorded string (this requires at most an additional $\poly\log(N)$ random bits of seed). \znote{Also need randomness for quantum extractor seed?}\tnote{yes, but no more than $\poly\log$. added comment}
...
starts with a cat state on $\poly\log(N)$ qubits, \znote{Fix polylog. What is a cat state?}\tnote{dude, did you not attend this Spring school? It's something like $\sum_i \ket{i}\ket{i}\ket{i}$, for any number of kets and were $i$ ranges over any range} and uses one of the registers of the state, $\reg{C}$, to provide the random bits used to select the test rounds and to issue the challenges in those rounds. We can similarly  arrange that 
...
introduced a particularly efficient implementation of a TCF family, based on the Learning with Errors problem (LWE). In this paper we use a similar construction, also based on the LWE problem.\footnote{A construction of trapdoor one-way functions with second pre-image resistance based on LWE was independently introduced in~\cite{cojocaru2018delegated}, where it is used to achieve delegated computation in the honest-but-curious model. The family of functions considered in~\cite{cojocaru2018delegated} is too weak for our purposes, as it lacks the essential adaptive hardcore bit property.}  \znote{If the previous paper already contained a TCF then what are we doing here? This is unclear.}\tnote{Urmila? Is it fair to call this a weaker variant of TCF family? How should we say it?}
