...
generate $O(N)$ bits that are statistically within negligible distance from uniform. However, this would require assuming that the underlying LWE assumption is hard even for sub-exponential size quantum circuits (which is consistent with current knowledge). The more conservative assumption that our variant of LWE is only hard for polynomial size quantum circuits will require use of $N^{\epsilon}$ bits of randomness for generating the TCF, for any constant $\epsilon >0$. The following is an informal statement; see Theorem~\ref{thm:expansion} for more details.
...
The protocol then proceeds for $N$ rounds, where $N$ is polynomial in the security parameter \znote{Polynomial or sub-exponential?}. In each round the device first outputs an image $y \in \{0,1\}^m$. After having received $y$, the verifier issues one of two 
...
uses a strong quantum-proof randomness extractor to extract $\Omega(N)$ bits of randomness from the recorded string. \znote{Also need randomness for quantum extractor seed?}
...
starts with a cat state on $\poly\log(N)$ qubits, \znote{Fix polylog. What is a cat state?} and uses one of the registers of the state, $\reg{C}$, to provide the random bits used to select the test rounds and to issue the challenges in those rounds. We can similarly  arrange that 
...
introduced a particularly efficient implementation of a TCF family, based on the Learning with Errors problem (LWE). In this paper we use a similar construction, also based on the LWE problem. \znote{If the previous paper already contained a TCF then what are we doing here? This is unclear.}
